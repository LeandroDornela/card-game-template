...
namespace CardGameTemplate
{
    ...
    {
        public static bool TryCreateRuntimeCardDefinition(CardDefinition cardDefinition, out RuntimeCardDefinition runtimeCardDefinition)
        {
            ICardBehaviour[] cardEffects;
        ...
        public static bool TryCreateRuntimeEffects(BehaviourDefinition[] effectDefinitions, out ICardBehaviour[] cardEffects)
        {
            // Check input effects array.
            if(effectDefinitions == null || effectDefinitions.Length == 0)
            {
                CardGameTemplate.Debug.LogError(Debug.Category.Data, $"{nameof(effectDefinitions)} has no elements.");
                cardEffects = null;
                return false;
            }
            
            // Fill effects array with new ICardEffects created from Card Definitions.
            cardEffects = new ICardBehaviour[effectDefinitions.Length];
        ...
        public static bool TryCreateRuntimeEffect(BehaviourDefinition effectDefinition, out ICardBehaviour cardEffect)
        {
